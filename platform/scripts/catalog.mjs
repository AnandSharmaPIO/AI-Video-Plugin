#!/usr/bin/env node
/**
 * catalog — inventory + staleness report over every feature in walkthroughs/.
 *
 * Usage:
...
for (const { project, module: mod, feature: feat, dir, yamlPath } of listFeatures()) {
  ...
  const mp4Path = path.join(dir, 'generated', 'walkthrough-narrated.mp4');
...
if (asJson) {
  ...
  console.log('No features found under walkthroughs/.');
