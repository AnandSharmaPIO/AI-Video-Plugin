...
Platform copy — serves ANY feature folder via --dir:
  <venv-python> platform/tts/generate.py --dir walkthroughs/<p>/modules/<m>/features/<f>
...
def load_lexicon(feature_dir: Path) -> "dict[str, str]":
    ...
    <project>/knowledge/lexicon.json → platform/tts/lexicon.default.json. Each
    ...
    for up in range(0, 6):
        ...
        for _ in range(up):
            base = base.parent
        knowledge = base / "knowledge" / "lexicon.json"
        if knowledge.exists():
