 ...
 * walkthrough. Reuses each feature's generated/final-video.mp4 (nothing
 ...
 *
 * Usage:
 *   node platform/scripts/journey.mjs <p>/journeys/<id> [flags]
...
if (positional.length !== 1) {
  console.error('usage: journey <<p>/journeys/<id>> [--produce-missing --env <e> --allow-mutations --out <mp4>]');
  process.exit(2);
}
...
// Derive the project from the path: <project>/journeys/<id>.
const relParts = path.relative(repoRoot, journeyDir).replace(/\\/g, '/').split('/');
if (relParts[1] !== 'journeys') {
  console.error(`✗ journey must live at <project>/journeys/<id> (got ${relParts.join('/')})`);
  process.exit(2);
}
...
function windowFor({ seg, featureDir }) {
  ...
  return {
    mp4: path.join(featureDir, 'generated', 'final-video.mp4'),
