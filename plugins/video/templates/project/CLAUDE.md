...
Playwright + local TTS. Point it at an app (source in `app-source/`, catalog at
`<id>/` — a project folder directly at the repo root) and it produces the
videos. Source code is only a lens
...
- `<id>/` — a project folder directly at the repo root (no fixed wrapper
  folder; any top-level directory containing a `project.yaml` is a project):
  `project.yaml`, `project-overview/` (app map, discovered by
  `/walkthrough-discover`), `modules/<m>/module.yaml`,
  `modules/<m>/features/<f>/` = `feature.yaml` (**source of truth**) +
  generated `walkthrough.spec.ts` + `narration.md`; media in gitignored
  `generated/` (final deliverable: `final-video.mp4`).
...
`<feature-path>` = `<p>/modules/<m>/features/<f>`.
...
- `/walkthrough-discover` — analyze `app-source/` + live app → `project-overview/`,
