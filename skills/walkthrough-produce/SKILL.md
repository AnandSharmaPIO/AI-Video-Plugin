...
`walkthroughs/<p>/modules/<m>/features/<f>`.
...
`walkthroughs/`) — the engine resolves the catalog from the current working
directory.
...
`dataSafety`. Use `walkthroughs/*/knowledge/` (routes, proven selectors, quirks)
...
quirk discovered, to `walkthroughs/<p>/knowledge/selectors.yaml` so the next
...
fix and every environment quirk you hit to `knowledge/selectors.yaml`** before
...
the output path `<feature>/generated/walkthrough-narrated.mp4` to the user.
...
feature's `generated/walkthrough-narrated.mp4`, so a feature video and the journey
stay in sync from one source. Author
`walkthroughs/<p>/journeys/<id>/journey.yaml` (schema:
...
```bash
/journey walkthroughs/<p>/journeys/<id>                       # reuse finished MP4s
/journey walkthroughs/<p>/journeys/<id> --produce-missing --env <e>   # produce stale/missing segments first
```
